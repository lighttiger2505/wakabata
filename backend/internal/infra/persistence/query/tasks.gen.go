...
func newTask(db *gorm.DB, opts ...gen.DOOption) task {
	...
	_task.Status = field.NewBool(tableName, "status")
...
type ITaskDo interface {
	...
	FilterWithNameAndRole(name string, role string) (result []model.Task, err error)
}

// SELECT * FROM @@table WHERE name = @name{{if role !=""}} AND role = @role{{end}}
func (t taskDo) FilterWithNameAndRole(name string, role string) (result []model.Task, err error) {
	var params []interface{}

	var generateSQL strings.Builder
	params = append(params, name)
	generateSQL.WriteString("SELECT * FROM tasks WHERE name = ? ")
	if role != "" {
