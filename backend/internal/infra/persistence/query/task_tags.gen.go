...
func newTaskTag(db *gorm.DB, opts ...gen.DOOption) taskTag {
	...
	_taskTag.ID = field.NewString(tableName, "id")
	_taskTag.TaskID = field.NewString(tableName, "task_id")
	_taskTag.TagID = field.NewString(tableName, "tag_id")
...
type ITaskTagDo interface {
	...
	FilterWithNameAndRole(name string, role string) (result []model.TaskTag, err error)
}

// SELECT * FROM @@table WHERE name = @name{{if role !=""}} AND role = @role{{end}}
func (t taskTagDo) FilterWithNameAndRole(name string, role string) (result []model.TaskTag, err error) {
	var params []interface{}

	var generateSQL strings.Builder
	params = append(params, name)
	generateSQL.WriteString("SELECT * FROM task_tags WHERE name = ? ")
	if role != "" {
