...
func newProject(db *gorm.DB, opts ...gen.DOOption) project {
	...
	_project.UserID = field.NewString(tableName, "user_id")
...
type IProjectDo interface {
	...
	FilterWithNameAndRole(name string, role string) (result []model.Project, err error)
}

// SELECT * FROM @@table WHERE name = @name{{if role !=""}} AND role = @role{{end}}
func (p projectDo) FilterWithNameAndRole(name string, role string) (result []model.Project, err error) {
	var params []interface{}

	var generateSQL strings.Builder
	params = append(params, name)
	generateSQL.WriteString("SELECT * FROM projects WHERE name = ? ")
	if role != "" {
