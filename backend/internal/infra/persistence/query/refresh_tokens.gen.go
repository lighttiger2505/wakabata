...
type IRefreshTokenDo interface {
	...
	FilterWithNameAndRole(name string, role string) (result []model.RefreshToken, err error)
}

// SELECT * FROM @@table WHERE name = @name{{if role !=""}} AND role = @role{{end}}
func (r refreshTokenDo) FilterWithNameAndRole(name string, role string) (result []model.RefreshToken, err error) {
	var params []interface{}

	var generateSQL strings.Builder
	params = append(params, name)
	generateSQL.WriteString("SELECT * FROM refresh_tokens WHERE name = ? ")
	if role != "" {
